...
.controller('CPQGroupController', ['$rootScope', '$scope' ,'$filter', '$window','CPQService','$sldsModal', '$timeout','$sldsToast','pageService', 'MultiSerivceCPQService',
    function($rootScope, $scope ,$filter, window,CPQService,$sldsModal, timeout,$sldsToast, pageService, MultiSerivceCPQService) {
        ...
        $scope.init = function() {
            ...
            if(groupedRecords && groupedRecords.length > 0) {
                ...
                for(let i = 0 ; i < $scope.groupList.length ; i++) {
                    ...
                    if (group.message) {
                        group.errorReport = JSON.parse(group.message);
                    }
         ...
         */
        $scope.getMembers = function(group, silentUpdate, customPageSize) {
            if(group.actions.getMembers) {
                var action = group.actions.getMembers;
                if(!silentUpdate) {
                ...
                MultiSerivceCPQService.invokeAction(group.actions.getMembers).then(
                    function(data) {
                        ...
                        if(data.records) {
                            ...
                            if(group.members) {
                                for(var i=0; i<group.members.length; i++) {
                                    ...
                                    if (group.members[i].cart) {
                                        var messageData = group.members[i].cart[$rootScope.nsPrefix + 'MultiSiteMessageData__c'];
                                        if (!messageData) {
                                            continue;
                                        }
                                        var parsedMsgDataList = JSON.parse(messageData);
                                        ...
                                        group.members[i].messageReport = messageReport;
                                    }
                                }
                            }
                            if(data.actions && data.actions.getMembers) {
                                action = data.actions.getMembers;
                            }
                            group.actions.getMembers = action;
                        }
                        if(!silentUpdate) {
         ...
         * inside a group.
         */
        $scope.getMoreMembers = function(group) {
            if(!group.actions || !group.actions.getMembers) {
                return;
            }
            group.isLoading = true;
            MultiSerivceCPQService.invokeAction(group.actions.getMembers).then(
            ...
            MultiSerivceCPQService.invokeRemote('getGroups', input).then(function(result){
                ...
                if(groupedRecords && groupedRecords.records) {
                    for(let i=0; i<groupedRecords.records.length; i++) {
                        ...
                        if(group) {
                            //group = Object.assign(group, gObj);
                            //merge values
                            ...
                            if(group.members && group.members.length > 0) {
                                $scope.getMembers(group, true, group.members.length);
                            }
                        }   
                    }
                }
...
.controller('GroupCPQTotalController', ['$scope', '$controller', '$sldsModal', 'pageService', 'CPQService', '$rootScope',
    function($scope, $controller, $sldsModal, pageService, CPQService, $rootScope) {
        ...
        var labelsArray = ['CPQApplyToGroupMessage', 'CPQClose', 'CPQApplyToGroup', 
            'CPQPriceValidateLabel', 'CPQPriceValidateMessage',
            'CPQMultiServiceCheckout', 'CPQCheckoutMessage'];
        CPQService.setLabels(labelsArray, $scope.customLabels);
        ...
        $scope.actionClicked = function(action) {
            ...
            } else if(action.Name === 'MultiServiceCheckout' || action.Name === 'MultiServiceCheckoutV2'){
        ...
        $scope.checkoutAction = function(action) {
            ...
            var modalScope = $scope.$new();
            modalScope.actionLabel = $scope.customLabels.CPQMultiServiceCheckout;
            modalScope.message = $scope.customLabels.CPQCheckoutMessage;
